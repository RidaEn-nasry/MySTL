...
/*   Updated: 2022/10/21 23:38:01 by ren-nasr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
  ...
  public:
    ...
    explicit map(const allocator_type& alloc = allocator_type(), const key_compare& comp = key_compare())
    ...
    map(InputIterator first, InputIterator last, const allocator_type& alloc = allocator_type(), const key_compare& comp = key_compare())
    {
      // _comp = value_compare(key_compare());
      // _comp(comp);
      (void)comp;
  ...
  private:
    ...
    node_type* _new_node(const value_type& val, node_type* parent, node_type* left = NULL, node_type* right = NULL)
    {
      ...
      _node_alloc.construct(new_node, node_type(val, comp, parent));
