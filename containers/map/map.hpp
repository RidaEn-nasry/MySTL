...
/*   Updated: 2022/10/22 12:40:49 by ren-nasr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
  ...
  public:
    ...
    explicit map(const allocator_type &alloc = allocator_type(), const key_compare &comp = key_compare()) : _alloc(alloc), _comp(comp), _size(0), _root(NULL), _min(NULL), _max(NULL){};

    // (2) range constructor
    template <class InputIterator>
    map(InputIterator first, InputIterator last, const allocator_type &alloc = allocator_type(), const key_compare &comp = key_compare()) : _alloc(alloc), _comp(comp), _size(0), _root(NULL), _min(NULL), _max(NULL)
    {
      insert(first, last);
    };
