...
#define MAP_HPP
  ...
  public:
    ...
    explicit map(const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _comp(comp), _alloc(alloc), _root(NULL), _min(NULL), _max(NULL), _size(0) {}

    // (2) range constructor
    template <class InputIterator>
    map(InputIterator first, InputIterator last, const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _comp(comp), _alloc(alloc), _root(NULL), _min(NULL), _max(NULL), _size(0) {
      insert(first, last);
    }

    // (3) copy constructor
    map(const map& x) : _comp(x._comp), _alloc(x._alloc), _root(NULL), _min(NULL), _max(NULL), _size(0) {
    ...
    private:
      ...
      // _inserting
      <iterator, bool> _insert(const value_type & val) {
        // neat insert
        node_type* old_max = _root;
